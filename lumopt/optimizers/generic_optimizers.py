...
class Optimizer(object):
    ...
    def initialize(
        self, start_params, callable_fom, callable_jac, bounds, plotting_function
    ):
        ...
        self.callable_fom, self.callable_jac = self.define_callables(
            callable_fom, callable_jac
        )
...
class FixedStepGradientDescent(Optimizer):
    ...
    def add_noise(self):
        noise = (
            self.noise_magnitude * (np.random.rand(len(self.current_params)) - 0.5) * 2
        )
    ...
    def run(self, verbose=True):
        ...
        while self.iteration < self.max_iter:
            ...
            while new_fom < self.fom_hist[-1] and self.dx > self.min_dx:
                ...
                new_params = self.current_params + self.calculate_change(
                    gradients, self.dx
                )
                new_params = self.enforce_bounds(new_params)
                new_fom = self.callable_fom(new_params)
...
class ScipyOptimizers(Optimizer):
    ...
    def run(self, verbose=True):
        ...
        res = sci.minimize(
            fun=self.callable_fom,
            x0=self.start_point,
            jac=self.callable_jac,
            bounds=self.bounds,
            callback=self.callback,
            options={"maxiter": self.max_iter, "disp": True, "gtol": self.pgtol},
            method=self.method,
        )
        return res
