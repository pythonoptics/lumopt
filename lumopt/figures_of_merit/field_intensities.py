...
class FieldIntensity(fom):
    ...
    def get_fom(self, simulation):
        ...
        pointfield = field.getfield(
            field.x[0], field.y[0], field.z[0], self.wavelengths[0]
        )
...
class FieldIntensities(fom):
    ...
    def __init__(
        self,
        positions=[(1.25e-6, 0.36e-6, 0), (1.25e-6, -0.36e-6, 0)],
        weight_amplitudes=[1, 1],
        wavelengths=[1550e-9],
        normalize_to_source_power=False,
    ):
        """
        ...
        self.monitor_names = [
            "fom_mon_{}".format(i) for i in range(len(self.positions))
        ]
    ...
    def get_fom(self, simulation):
        ...
        pointfields = [
            field.getfield(field.x[0], field.y[0], field.z[0], self.wavelengths[0])
            for field in fields
        ]
        if self.weight_amplitudes is None:
            fom = sum(
                [sum(pointfield * np.conj(pointfield)) for pointfield in pointfields]
            )
        else:
            sum_of_pointfields = sum(
                [
                    pointfield * phase_factor
                    for pointfield, phase_factor in zip(
    ...
    def add_adjoint_sources(self, sim):
        ...
        if self.weight_amplitudes is None:
            adjoint_sources = [
                prefactor * np.conj(pointfield) for pointfield in pointfields
            ]
        else:
            pointfields = [
                field.getfield(field.x[0], field.y[0], field.z[0], self.wavelengths[0])
            ...
            adjoint_sources = [
                prefactor * phase_factor for phase_factor in self.weight_amplitudes
            ]
