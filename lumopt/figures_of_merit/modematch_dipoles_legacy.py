...
class Modematch_field_source(ModeMatch):
    ...
    def add_adjoint_sources(self, simulation):
        ...
        adjoint_injection_mode = Fields(
            ...
            self.mode.z,
            self.mode.wl,
            self.mode.E * np.conj(pp) * omega * 1j,
            self.mode.D,
            self.mode.eps,
...
class ModeMatch2(fom):
    ...
    def initialize(self, sim, monitor_name="fom", plot=False):
        ...
        if len(self.mode.z) != 1:
            ...
            z_dip = np.linspace(
                min(self.mode.z) + z_span / 1000,
                max(self.mode.z) - z_span / 1000,
                self.precision,
            )
            dz = z_dip[1] - z_dip[0]
        else:
        ...
        if len(self.mode.x) != 1:
            ...
            x_dip = np.linspace(
                min(self.mode.x) + x_span / 1000,
                max(self.mode.x) - x_span / 1000,
                self.precision,
            )
            dx = x_dip[1] - x_dip[0]
        else:
        ...
        if len(self.mode.y) != 1:
            ...
            y_dip = np.linspace(
                min(self.mode.y) + y_span / 1000,
                max(self.mode.y) - y_span / 1000,
                self.precision,
            )
            dy = y_dip[1] - y_dip[0]
        else:
        ...
        for x in x_dip:
            for z in z_dip:
                for y in y_dip:
                    ...
                    Em = Em_interpolator(x, y, z, self.wavelength)
                    phase_factor = (
                        np.conj([0, Hm[2], -Hm[1]]) / 2
                    )  # for mode overlap calculation, and because the field is only being calculated in one direction
                    phase_factors.append(phase_factor * dy * dx * dz)

        self.field_intensities_fom_object = FieldIntensities(
            positions=positions,
            weight_amplitudes=phase_factors,
            normalize_to_source_power=True,
        )
...
class ModeMatch3(ModeMatch2):
    ...
    def get_fom(self, simulation):
        ...
        sum_of_pointfields = sum(
            [
                pointfield * phase_factor
                for pointfield, phase_factor in zip(pointfields, self.phase_factors)
            ]
        )
    ...
    def add_adjoint_sources(self, simulation):
        ...
        adjoint_sources = [
            prefactor * phase_factor for phase_factor in self.phase_factors
        ]
