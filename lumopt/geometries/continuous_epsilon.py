...
class ContinousEpsilon2D(Geometry):
    ...
    def calculate_gradients(self, gradient_fields, wavelength, real=True):
        ...
        for x in self.x:
            for y in self.y:  # ,y in zip(xx.reshape(-1),yy.reshape(-1)):
                derivs.append(
                    gradient_fields.integrate_square(
                        center=(x, y), box=(dx, dy), z=self.z, wl=wavelength, real=real
                    )
                )
            print(".", end=" ")
        print("")
    ...
    def update_geometry(
        self, params
    ):  # here params is really just a linearized version of the epsilon map
...
class FunctionDefinedContinuousEpsilon2D(ContinousEpsilon2D):
    def __init__(
        self, func, initial_params, bounds, z=0, depth=220e-9, dx=1e-15, addmesh=True
    ):
    ...
    def calculate_gradients(self, gradient_fields, wavelength, real=True):
        ...
        deriv_map = super(FunctionDefinedContinuousEpsilon2D, self).calculate_gradients(
            gradient_fields, wavelength, real=False
        )
...
class FunctionDefinedContinuousEpsilon3DYeeGrid(Geometry):
    ...
    def get_eps_derivatives(self):
        ...
        for eps_derivative in eps_derivatives:
            ...
            eps_derivative_x[:-1, :, :] = (
                eps_derivative[:-1, :, :] + eps_derivative[1:, :, :]
            ) / 2
            eps_derivative_y[:, :-1, :] = (
                eps_derivative[:, :-1, :] + eps_derivative[:, 1:, :]
            ) / 2
            ...
            eps_derivatives_Yee.append(
                np.stack(
                    (eps_derivative_x, eps_derivative_y, eps_derivative_z), axis=-1
                )
            )

        return (
            eps_derivatives_Yee
        )  # np.stack((eps_derivatives_x,eps_derivatives_y,eps_derivatives_z),axis=-1)
    ...
    def calculate_gradients(self, gradient_fields, wavelength, real=True):
        ...
        for eps_derivative in eps_derivatives:
            if real:
                derivs.append(
                    np.real(np.sum(sp_field * eps_derivative))
                    * (self.x[1] - self.x[0])
                    * (self.y[1] - self.y[0])
                    * (self.z[1] - self.z[0])
                )
            else:
                derivs.append(np.sum(sp_field * eps_derivative)) * (
                    self.x[1] - self.x[0]
    ...
    def calculate_gradients(self, gradient_fields, wavelength, real=True):
        ...
        for eps_derivative in eps_derivatives:
            if real:
                ...
                derivs.append(
                    np.sum(sp_field * eps_derivative)
                    * (self.x[1] - self.x[0])
                    * (self.y[1] - self.y[0])
                    * (self.z[1] - self.z[0])
                )
...
if __name__ == "__main__":
    ...
    script = load_from_lsf(
        os.path.join(
            CONFIG["root"], "examples/staight_waveguide/straight_waveguide.lsf"
        )
    )
    ...
    eps[90, 10] = 10
    geometry = ContinousEpsilon2D(
        eps=eps, x=np.linspace(-1e-6, 1e-6, nx), y=np.linspace(-0.4e-6, 0.4e-6, ny)
    )
