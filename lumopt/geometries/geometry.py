...
class Geometry(object):
    ...
    def __init__(self, geometries, operation):
        ...
        if self.operation == "add":
            self.bounds = np.concatenate(
                (np.array(geometries[0].bounds), np.array(geometries[1].bounds))
            )
    ...
    def calculate_gradients(self, gradient_fields, wavelength, real=True):
        derivs1 = np.array(
            self.geometries[0].calculate_gradients(
                gradient_fields, wavelength, real=real
            )
        )
        derivs2 = np.array(
            self.geometries[1].calculate_gradients(
                gradient_fields, wavelength, real=real
            )
        )
    ...
    def calculate_gradients_from_sims_eps(self, gradient_fields, real=True):
        ...
        for d_eps in d_epses:
            deriv = trapz3D(
                np.sum(
                    gradient_fields.sparse_perturbation_field_nosum * d_eps, axis=-1
                )[:, :, :, 0],
                x,
                y,
                z,
