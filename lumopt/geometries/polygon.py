...
class Polygon(Geometry):
    ...
    def __init__(
        self,
        points=np.array([(1, 1), (-1, 1), (-1, -1), (1, -1)]) * 0.05e-6,
        z=0,
        depth=100e-9,
        eps_out=2 ** 2,
        eps_in=3.44 ** 2,
...
class function_defined_Polygon(Polygon):
    ...
    def calculate_gradients(self, gradient_fields, wavelengths, real=True):
        ...
        wavelength = wavelengths[
            0
        ]  # TODO THIS IS NOT DEALING WITH MULTIPLE WAVELENGTHS
        polygon_gradients = np.array(
            super(function_defined_Polygon, self).calculate_gradients(
                gradient_fields, wavelength, real=real
            )
        )
    ...
    def add_poly_script(self, points, only_update=False):
        ...
        vertices_string = np.array2string(
            points, max_line_width=10e10, floatmode="unique", separator=","
        ).replace(",\n", ";")
        if not only_update:
            script = "addpoly;" "set('name','polygon_{0}');".format(self.hash)
        else:
