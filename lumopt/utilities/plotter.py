...
class Plotter(object):
    ...
    def update(self, optimization):
        optimization.optimizer.plot(
            fomax=self.ax[0, 0], paramsax=self.ax[0, 1], gradients_ax=self.ax[0, 2]
        )
        ...
        if isinstance(optimization, Optimization):
            ...
            if not optimization.optimizations[0].geometry.plot(self.ax[1, 0]):
                try:
                    optimization.optimizations[0].gradient_fields.plot_eps(
                        self.ax[1, 0]
                    )
                except:
                    print("can't plot geometry")
            try:
                optimization.optimizations[0].gradient_fields.plot(
                    self.fig, self.ax[1, 1], self.ax[1, 2]
                )
            except:
