...
def trapz3D(vals, x, y, z):
    ...
    x, y, z = list(map(process_input, [x, y, z]))
...
def dblsimps(func, x_min, x_max, y_min, y_max, points_per_dimension=1):
    ...
    if points_per_dimension == 1:
        return (
            (x_max - x_min)
            * (y_max - y_min)
            * func((x_max + x_min) / 2, (y_max + y_min) / 2)
        )
    else:
...
def wrapped_GridInterpolator(
    points, values, method="linear", bounds_error=True, fill_value=float("nan")
):
    ...
    *nearest*) may be chosen at each evaluation.
    ...
    dim_1_inputs = [
        array.size == 1 for array in points
    ]  # find all 1 dimensional values
