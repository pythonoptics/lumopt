...
class Edge(object):
    ...
    def derivative_3D(self, gradient_fields, wl, n_points, real=True):
        ...
        edge_derivative_layers = [
            self.derivative_2D(gradient_fields, wl, n_points, z_override=z, real=real)
            for z in z_s
        ]
    ...
    def derivative_2D(self, gradient_fields, wl, n_points, z_override=None, real=True):
        ...
        integration_points = [
            self.first_point * i + self.second_point * (1 - i)
            for i in np.linspace(1, 0, n_points)
        ]
        ...
        integrand_points = [
            integrand(
                ...
                wl,
                self.normal,
                self.eps_in.get_eps(wl),
                self.eps_out.get_eps(wl),
            )
        ...
        deriv_first = trapz1D(
            integrand_points * weights, self.length * np.linspace(0, 1, n_points)
        )

        # and now the second point
        weights = np.linspace(0, 1, n_points)
        deriv_second = trapz1D(
            integrand_points * weights, self.length * np.linspace(0, 1, n_points)
        )
