...
class Fields(object):
    ...
    def make_field_interpolation_object(self, F):
        ...
        Fx_interpolator = wrapped_GridInterpolator(
            (self.x, self.y, self.z, self.wl), F[:, :, :, :, 0], method="linear"
        )
        Fy_interpolator = wrapped_GridInterpolator(
            (self.x, self.y, self.z, self.wl), F[:, :, :, :, 1], method="linear"
        )
        Fz_interpolator = wrapped_GridInterpolator(
            (self.x, self.y, self.z, self.wl), F[:, :, :, :, 2], method="linear"
        )
        ...
        def field_interpolator(x, y, z, wl):
            ...
            return np.array(
                (Fx, Fy, Fz)
            ).squeeze()  # TODO: fix this! This squeeze is a mistery when matlab is used as midman...
    ...
    def calculate_pointing_vect(self):
        ...
        if self.E is None or self.H is None:
            return ValueError(
                "Either E or H data is missing in the field, cannot calculate pointing vector"
            )

        pointing_vect = np.zeros(self.E.shape, dtype=np.complex_)

        pointing_vect[:, :, :, :, 0] = np.multiply(
            self.E[:, :, :, :, 1], np.conj(self.H[:, :, :, :, 2])
        ) - np.multiply(self.E[:, :, :, :, 2], np.conj(self.H[:, :, :, :, 1]))
        pointing_vect[:, :, :, :, 1] = np.multiply(
            self.E[:, :, :, :, 2], np.conj(self.H[:, :, :, :, 0])
        ) - np.multiply(self.E[:, :, :, :, 0], np.conj(self.H[:, :, :, :, 2]))
        pointing_vect[:, :, :, :, 2] = np.multiply(
            self.E[:, :, :, :, 0], np.conj(self.H[:, :, :, :, 1])
        ) - np.multiply(self.E[:, :, :, :, 1], np.conj(self.H[:, :, :, :, 0]))
    ...
    def calculate_overlap(self, other_field, remove_E=False, remove_H=False):
        ...
        if not (
            len(self.x) == len(other_field.x)
            and len(self.y) == len(other_field.y)
            and len(self.z) == len(other_field.z)
            and len(self.wl) == len(other_field.wl)
        ):
            raise ValueError(
                "Fields are not on same grid, (or not the same amount of wavelengths Modematch does not support this (write a method!!)"
            )
        ...
        # E x H calculation:
        if remove_E:
            integrand[:, :, :, :, 0] = np.multiply(
                np.conj(self.E[:, :, :, :, 1]), other_field.H[:, :, :, :, 2]
            ) - np.multiply(
                np.conj(self.E[:, :, :, :, 2]), other_field.H[:, :, :, :, 1]
            )
            integrand[:, :, :, :, 1] = np.multiply(
                np.conj(self.E[:, :, :, :, 2]), other_field.H[:, :, :, :, 0]
            ) - np.multiply(
                np.conj(self.E[:, :, :, :, 0]), other_field.H[:, :, :, :, 2]
            )
            integrand[:, :, :, :, 2] = np.multiply(
                np.conj(self.E[:, :, :, :, 0]), other_field.H[:, :, :, :, 1]
            ) - np.multiply(
            ...
            integrand[:, :, :, :, 0] = np.multiply(
                other_field.E[:, :, :, :, 1], np.conj(self.H[:, :, :, :, 2])
            ) - np.multiply(
                other_field.E[:, :, :, :, 2], np.conj(self.H[:, :, :, :, 1])
            )
            integrand[:, :, :, :, 1] = np.multiply(
                other_field.E[:, :, :, :, 2], np.conj(self.H[:, :, :, :, 0])
            ) - np.multiply(
                other_field.E[:, :, :, :, 0], np.conj(self.H[:, :, :, :, 2])
            )
            integrand[:, :, :, :, 2] = np.multiply(
                other_field.E[:, :, :, :, 0], np.conj(self.H[:, :, :, :, 1])
            ) - np.multiply(
        ...
        for i, wl in enumerate(self.wl):
            ...
            power[i] = (
                np.abs(amplitude_prefactor) ** 2 / 16
            )  # The factor of 16 is different from eq 7.5 of Keraly et al. but with the mode normalization it should be ok
            if remove_E or remove_H:
                amplitude_prefactor = (
                    amplitude_prefactor * 2
                )  # not sure why, could have to do with the import H
            amplitude_prefactors[i] = (
                amplitude_prefactor / 16
            )  # for the phase of injection in the adjoint sim
    ...
    def plot_full(
        self, D=False, E=True, eps=False, H=False, wl=1550e-9, original_grid=True
    ):
        ...
        if D:
            self.plot_field(
                self.getDfield, original_grid=original_grid, wl=wl, name="D"
            )
        if eps:
            self.plot_field(self.geteps, original_grid=original_grid, wl=wl, name="eps")
        if H:
            self.plot_field(
                self.getHfield, original_grid=original_grid, wl=wl, name="H"
            )
...
class FieldsNoInterp(Fields):
    def __init__(self, x, y, z, wl, deltas, E, D, eps, H):
        ...
        x, y, z, wl, delta_x, delta_y, delta_z = list(
            map(process_input, [x, y, z, wl, delta_x, delta_y, delta_z])
        )
    ...
    def make_field_interpolation_object_nointerp(self, F):
        ...
        Fx_interpolator = wrapped_GridInterpolator(
            (self.x + self.deltas[0], self.y, self.z, self.wl),
            F[:, :, :, :, 0],
            method="linear",
            bounds_error=False,
        )
        Fy_interpolator = wrapped_GridInterpolator(
            (self.x, self.y + self.deltas[1], self.z, self.wl),
            F[:, :, :, :, 1],
            method="linear",
            bounds_error=False,
        )
        Fz_interpolator = wrapped_GridInterpolator(
            (self.x, self.y, self.z + self.deltas[2], self.wl),
            F[:, :, :, :, 2],
            method="linear",
            bounds_error=False,
        )
