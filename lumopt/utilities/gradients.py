...
class Gradient_fields(object):
    ...
    def __init__(self, forward_fields, adjoint_fields):
        ...
        self.sparse_perturbation_field_function_leg = (
            self.def_sparse_function()
        )  # legacy
    ...
    def def_pre_computed_sparse(self):
        ...
        self.sparse_perturbation_field_nosum = (
            2 * eps0 * self.forward_fields.E * self.adjoint_fields.E
        )
        self.sparse_perturbation_field = np.sum(
            2 * eps0 * self.forward_fields.E * self.adjoint_fields.E, axis=-1
        )
        interpolator = wrapped_GridInterpolator(
            (
            ...
            self.sparse_perturbation_field,
            method="linear",
        )

        def gradient_field(x, y, z, wl, real=True):
            if real:
                return np.real(interpolator((x, y, z, wl)))[
                    0
                ]  # Eq 5.28 of O. Miller's thesis
            else:
        ...
        def gradient_field(x, y, z, wl, real=True):
            ...
            if real:
                return sum(
                    ...
                    * eps0
                    * np.real(
                        self.forward_fields.getfield(x, y, z, wl)
                        * self.adjoint_fields.getfield(x, y, z, wl)
                    )
                )  # Eq 5.28 of O. Miller's thesis
            else:
                return sum(
        ...
        def gradient_field(x, y, z, wl, normal, eps_in, eps_out):
            ...
            if real:
                return sum(
                    2
                    * np.real(
                        (eps0 * eps_in - eps0 * eps_out)
                        * E_parallel_forward
                        * E_parallel_adjoint
                        + (1.0 / (eps0 * eps_out) - 1.0 / (eps0 * eps_in))
                        * D_perp_forward
                        * D_perp_adjoint
                    )
                )  # Eq 5.28 of O. Miller's thesis
    ...
    def plot_eps(self, ax_eps):
        ...
        im = ax_eps.pcolormesh(
            xx * 1e6, yy * 1e6, np.real(np.transpose(eps))
    ...
    def plot_gradients(self, fig, ax_gradients, original_grid=True):
        ...
        Sparse_pert = [
            self.sparse_perturbation_field_function_leg(x, y, z, wl)
            for x, y in zip(xx, yy)
        ]

        im = ax_gradients.pcolormesh(
            xx * 1e6, yy * 1e6, Sparse_pert, cmap=plt.get_cmap("bwr")
        )
        # fig.colorbar(im,ax=ax_gradients)
    ...
    def integrate_square(self, center, box, z, wl, real=True):
        ...
        xmin = center[0] - box[0] / 2.0
        xmax = center[0] + box[0] / 2.0
        ymin = center[1] - box[1] / 2.0
        ymax = center[1] + box[1] / 2.0

        integrand = lambda x, y: self.sparse_perturbation_field_function(
            x, y, z, wl, real=real
        )
    ...
    def integrate_cube(self, center, box, depth, wl, trap_method=True):
        ...
        x_min = center[0] - box[0] / 2.0
        x_max = center[0] + box[0] / 2.0
        y_min = center[1] - box[1] / 2.0
        y_max = center[1] + box[1] / 2.0
        z_min = center[2] - depth / 2.0
        z_max = center[2] + depth / 2.0
