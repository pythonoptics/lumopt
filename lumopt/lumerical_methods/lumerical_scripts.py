...
def remove_interpolation_on_monitor(fdtd, monitor_name="opt_fields"):
    ...
    script = "select({});" "set('spatial interpolation','none');".format(monitor_name)
    fdtd.eval(script)
...
def get_fields_no_interp(fdtd, monitor_name, get_eps=False, get_D=False, get_H=False):
    ...
    delta_z = (
        np.array(0)
        if fdtd.getresult(monitor_name, "dimension") == 2.0
        else fdtd.getresult(monitor_name, "delta_z")
    )
    deltas = [delta_x, delta_y, delta_z]
    if get_D:
...
def get_fields_interp(fdtd, monitor_name, get_eps=False, get_D=False, get_H=False):
    ...
    fields = fdtd.getresult(monitor_name, "E")
    fields_eps = (
        fdtd.getresult(monitor_name + "_D_index", "eps")["eps"] if get_eps else None
    )
...
def get_fields(
    fdtd, monitor_name, get_eps=False, get_D=False, get_H=False, nointerpolation=False
):
    """This function fetches fields from a monitor.
    For the fields used to create gradients, the D component of the field and the refractive index also need to be fetched.
    For the fields on figure of merit monitors, D  or index does not need to be fetched
...
def set_spatial_interp(fdtd, monitor_name, setting):
    script = 'select("{}");set("spatial interpolation","{}");'.format(
        monitor_name, setting
    )
    fdtd.eval(script)
...
def add_imported_source(
    fdtd,
    fields,
    sourcename="imported_source",
    fommonitorname="fom",
    wavelengths=[1.55e-6],
    direction="Forward",
):
    ...
    copy_properties(
        ...
        fommonitorname,
        sourcename,
        properties=["x", "y", "z", "x_span", "y_span", "z_span"],
    )
...
def copy_properties(
    fdtd, origin, destination, properties=["x", "y", "z", "x_span", "y_span", "z_span"]
):
    ...
    for thing in properties:
        try:
            dest.__setattr__(
                thing.replace(" ", "_"), orig.__getattr__(thing.replace(" ", "_"))
            )
        except:
...
def set_injection_axis(fdtd, source_name):
    ...
    if 0 == src.z_span:
        src.injection_axis = "z-axis"
    if 0 == src.y_span:
        src.injection_axis = "y-axis"
    if 0 == src.x_span:
        src.injection_axis = "x-axis"
...
def add_D_monitors_to_fields_monitors(fdtd, monitor_name):
    ...
    fdtd.eval(
        'addobject("displacement_field_adv"); set("name","{}");'.format(
            index_monitor_name
        )
    )
...
def add_index_to_fields_monitors(fdtd, monitor_name):
    ...
    copy_properties(
        ...
        monitor_name,
        index_monitor_name,
        ["monitor type", "x", "y", "z", "x span", "y span", "z span"],
    )
...
def add_dipole(fdtd, x, y, z, wavelength, dipole, name_suffix="", magnetic=False):
    ...
    lumerical_script = add_dipole_script(
        x, y, z, wavelength, dipole, name_suffix, magnetic
    )
...
def add_dipole_script(x, y, z, wavelength, dipole, name_suffix="", magnetic=False):
    ...
    for dir, phasor in zip(["x", "y", "z"], dipole):
        ...
        if not amplitude == 0:
            ...
            if magnetic:
                lumerical_script += "set('dipole type','Magnetic dipole');"
            lumerical_script += (
                "set('name','{8}');"
                "set('x',{0});"
                "set('y',{1});"
                ...
                "set('amplitude',{7}/baseAmp);".format(
                    x,
                    y,
                    z,
                    theta,
