from lumopt.lumerical_methods.lumerical_scripts import (
    add_D_monitors_to_fields_monitors,
    remove_interpolation_on_monitor,
    add_index_to_fields_monitors,
    set_spatial_interp,
)
...
class Optimization(Super_Optimization):
    ...
    def __init__(
        self, base_script, fom, geometry, optimizer, plotter=Plotter(), use_deps=False
    ):
        """
        :param script:
    ...
    def calculate_finite_differences_gradients_2(
        self,
        n_derivatives=list(range(4, 6)),
        dx=0.01e-9,
        central=False,
        print_res=True,
        superverbose=False,
        ...
        for i, param in zip(n_derivatives, params[n_derivatives]):
            if not central:
                ...
                plt.show()
                recalculated_adjoint_deriv = trapz3D(
                    np.sum(sparse_pert_E * d_eps, axis=-1)[:, :, :, 0],
                    self.forward_fields.x,
                    self.forward_fields.y,
                    self.forward_fields.z,
                )
                print(
                    "recalculated adjoint gradients={}".format(
                        recalculated_adjoint_deriv
                    )
                )
            ...
            if print_res:
                print(("Derivative n {}={}".format(i, deriv)))
        self.geometry.update_geometry(params)
    ...
    def calculate_finite_differences_gradients(
        self,
        n_derivatives=list(range(4, 6)),
        dx=3e-9,
        central=True,
        ...
        if self.geometry.self_update:
            finite_differences_gradients = (
                self.geometry.calculate_finite_differences_gradients()
            )
        else:
            finite_differences_gradients = []
            try:  # If the geometry knows how to get it's own finite differences
                finite_differences_geometries = self.geometry.get_geometries_for_finite_differences_gradients(
                    dx=dx
                )
                current_fom = self.get_fom_geo(self.geometry)
                for i, geometry in enumerate(
                    finite_differences_geometries[:n_derivatives]
                ):
                ...
                for i, param in zip(n_derivatives, params[n_derivatives]):
                    if not central:
                        ...
                        if superverbose:
                            print(("dfom + ={}".format(d_fom_pos)))
                        print(("getting - "), end=" ")
                        d_fom_neg = self.callable_fom(d_params_neg)
                        if superverbose:
                            print(("dfom + ={}".format(d_fom_neg)))
                        deriv = (d_fom_pos - d_fom_neg) / dx / 2.0
                        finite_differences_gradients.append(deriv)
    ...
    def calculate_gradients(self, real=True):
        ...
        self.gradient_fields = Gradient_fields(
            forward_fields=self.forward_fields, adjoint_fields=self.adjoint_fields
        )

        "Let the geometry calculate the actual gradients"
        if not self.use_deps:
            self.gradients = self.geometry.calculate_gradients(
                self.gradient_fields, self.fom.wavelengths, real=real
            )
        else:
            self.gradients = self.geometry.calculate_gradients_from_sims_eps(
                self.gradient_fields, real=real
            )
    ...
    def goto_new_sims_folder(self):
        ...
        try:
            old = max(
                [int(x.split("_")[-1]) for x in directories if x.startswith("opts_")]
            )
...
if __name__ == "__main__":
    ...
    base_script = load_from_lsf(
        os.path.join(
            CONFIG["root"], "examples/crossing/crossing_base_TE_modematch_2D.lsf"
        )
    )
    ...
    opt = Optimization(
        base_script=base_script, fom=fom, geometry=geometry, optimizer=optimizer
    )
