...
script_1550 = load_from_lsf(
    os.path.join(CONFIG["root"], "examples/WDM_splitter/WDM_splitter_base_TE_1550.lsf")
)
script_1310 = load_from_lsf(
    os.path.join(CONFIG["root"], "examples/WDM_splitter/WDM_splitter_base_TE_1550.lsf")
).replace("1550e-9", "1310e-9")
...
def lower_coupler_arm(params, n_points=10):
    points_x = np.concatenate(
        ([0.5e-6], np.linspace(0.55e-6, size_x - 0.55e-6, 20), [size_x - 0.5e-6])
    )
    points_y = np.concatenate(
        (
            [-separation / 2],
            params - separation / 2,
            params[::-1] - separation / 2,
            [-separation / 2],
        )
    )
    ...
    polygon_points_y = [
        max(min(point, 0e-6), -separation / 2 - 0.25e-6)
        for point in interpolator(polygon_points_x)
    ]

    polygon_points_up = [(x, y) for x, y in zip(polygon_points_x, polygon_points_y)]
    polygon_points_down = [
        (x, y - 0.5e-6) for x, y in zip(polygon_points_x, polygon_points_y)
    ]
...
def upper_coupler_arm(params, n_points=10):
    ...
    interpolator = scipy.interpolate.interp1d(points_x, points_y, kind="cubic")
    polygon_points_y = [
        max(min(point, separation / 2 + 0.5e-6), -0e-6)
        for point in interpolator(polygon_points_x)
    ]
...
geometry_1550_lower = function_defined_Polygon(
    func=lower_coupler_arm,
...
geometry_1550_upper = function_defined_Polygon(
    func=upper_coupler_arm,
    initial_params=initial_params,
    eps_out=1.44 ** 2,
    eps_in=2.8 ** 2,
    bounds=bounds,
...
geometry_1310_lower = function_defined_Polygon(
    func=lower_coupler_arm,
    initial_params=initial_params,
    eps_out=1.44 ** 2,
    eps_in=2.8 ** 2,
...
geometry_1310_upper = function_defined_Polygon(
    func=upper_coupler_arm,
    initial_params=initial_params,
    eps_out=1.44 ** 2,
    eps_in=2.8 ** 2,
...
opt_1550 = Optimization(
    base_script=script_1550,
    fom=fom_1550,
    geometry=geometry_1550,
    optimizer=optimizer_1550,
)
opt_1310 = Optimization(
    base_script=script_1310,
    fom=fom_1310,
