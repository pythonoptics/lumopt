...
script = load_from_lsf(
    os.path.join(CONFIG["root"], "examples/grating/grating_base_continuous.lsf")
)
...
geometry = ContinousEpsilon2D(
    eps=eps, x=np.linspace(0, 18e-6, x_points), y=np.linspace(0, 0.22e-6, y_points)
)
...
optimizer = ScipyOptimizers(
    max_iter=200, method="L-BFGS-B", scaling_factor=1e-6, pgtol=1e-10
)
