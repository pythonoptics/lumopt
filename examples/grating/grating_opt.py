...
def grate_function_generator(
    grate_number, grate_height=70e-9, grate_center_y=220e-9 - 35e-9
):
...
first_tooth = function_defined_Polygon(
    func=grate_function_generator(0),
...
for i in range(1, n_grates):
    new_tooth = function_defined_Polygon(
        func=grate_function_generator(i),
        initial_params=initial_params,
        eps_out=silicon,
        eps_in=oxide,
        bounds=bounds,
...
fom = ModeMatch(
    modeorder=1, monitor_name="fom", wavelength=1550e-9
)  # ,direction='Backward')
...
opt = Optimization(
    base_script=script, fom=fom, geometry=full_geometry, optimizer=optimizer
)
